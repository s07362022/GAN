...
### data 
D = "D:\\harden\\02\\simpsons_dataset\\bart_simpson\\"   #####path
images = []
labels = []
dir_counts = 0
IMAGE_SIZE = 64
...
def anomaly_detection(test_img, g=None, d=None):
    ...
    ano_score, similar_img = anogan.compute_anomaly_score(model, test_img.reshape(1, 64, 64, 1), iterations=500, d=d)

    # anomaly area, 255 normalization
    # np_residual = test_img.reshape(28,28,1) - similar_img.reshape(28,28,1)
    np_residual = test_img.reshape(64,64,1) - similar_img.reshape(64,64,1)
    ...
    original_x = (test_img.reshape(64,64,1)*127.5+127.5).astype(np.uint8)
    similar_x = (similar_img.reshape(64,64,1)*127.5+127.5).astype(np.uint8)
...
random_image = np.random.uniform(0, 1, (100, 64, 64, 1))
#random_image = np.random.uniform(0, 1, (100, 28, 28, 1))
print("random noise image")
plt.figure(4, figsize=(2, 2))
plt.title('random noise image')
plt.imshow(random_image[0].reshape(64,64), cmap=plt.cm.gray)
